...
export function DocumentDocxPreview({ documentId, refreshKey = 0 }: DocumentDocxPreviewProps) {
  ...
  return (
    <div
      className="document-docx-preview"
      style={{
...
interface DocumentDocxBlobPreviewProps {
  blob: Blob
  refreshKey?: string | number
}

export function DocumentDocxBlobPreview({ blob, refreshKey = 0 }: DocumentDocxBlobPreviewProps) {
  const { token } = theme.useToken()
  const { t } = useTranslation()
  const containerRef = useRef<HTMLDivElement>(null)
  const [isLoading, setIsLoading] = useState(true)
